...
const TIERS = ["ADULT", "CHILD", "DIVER", "NON_DIVER"];
...
function rowToBoatForm(b: BoatRow): BoatFormData {
  const form = emptyBoatForm(TIERS, "LAND_TOUR");
  form.name = b.name; form.companyId = b.companyId; form.type = "LAND_TOUR";
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function LandTourPage() {
  ...
  const [boatForm, setBoatForm] = useState<BoatFormData>(emptyBoatForm(TIERS, "LAND_TOUR"));
  ...
  const load = useCallback(async () => {
    const [coData, boatData] = await Promise.all([
      ...
      fetch("/api/boats?type=LAND_TOUR").then(r => r.json()).catch(() => []),
    ]);
  ...
  // ─ Boat CRUD ─
  const openNewBoat = () => { setBoatForm(emptyBoatForm(TIERS, "LAND_TOUR")); setEditBoatId(null); setBoatOpen(true); };
  ...
  const openEditSched = (s: ScheduleRow) => {
    ...
    for (const tr of s.translations ?? []) {
      if (ALL_LANGS.includes(tr.lang as LangKey))
        (base as Record<string, unknown>)[tr.lang] = { title: tr.title, slug: tr.slug, excerpt: tr.excerpt, content: tr.content, itinerary: tr.itinerary, keywords: tr.keywords ?? [] };
    }
  ...
  return (
    <div style={{ padding: "24px 20px", maxWidth: 960 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#e5e5e5" }}>🗺 Land Tour</h1>
      ...
      {boats.length === 0 && (
        <div style={{ textAlign: "center", color: "#333", padding: "60px 0", fontSize: 14 }}>
          ยังไม่มี Land Tour — กด + เพิ่ม เพื่อเริ่ม
        </div>
      )}
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "12px 14px" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                    ...
                    <button title="Packages" onClick={() => setPkgBoat({ id: boat.id, name: boat.translations.find(t => t.lang === "en")?.title || boat.name })} style={{ background: "none", border: "1px solid #222", color: "#fff", borderRadius: 6, padding: "5px 7px", cursor: "pointer", display: "flex", alignItems: "center" }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex" }}>
          ...
          <div style={{ width: "min(720px,100%)", background: "#0d0d0d", borderLeft: "1px solid #1a1a1a", overflowY: "auto", display: "flex", flexDirection: "column" }}>
            <div style={{ position: "sticky", top: 0, zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "#0d0d0d", borderBottom: "1px solid #111" }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: "#ddd" }}>{editBoatId ? "แก้ไข Land Tour" : "เพิ่ม Land Tour"}</span>
