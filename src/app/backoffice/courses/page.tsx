...
function rowToBoatForm(b: BoatRow): BoatFormData {
  const form = emptyBoatForm(TIERS, "SCUBA_COURSES" as "DAYTRIP");
  form.name = b.name; form.companyId = b.companyId; form.type = "SCUBA_COURSES" as "DAYTRIP";
...
export default function ScubaCoursesPage() {
  ...
  const [boatForm, setBoatForm] = useState<BoatFormData>(emptyBoatForm(TIERS, "SCUBA_COURSES" as "DAYTRIP"));
  ...
  const load = useCallback(async () => {
    const [coData, boatData] = await Promise.all([
      ...
      fetch("/api/boats?type=SCUBA_COURSES").then(r => r.json()).catch(() => []),
    ]);
  ...
  const openNewBoat = () => { setBoatForm(emptyBoatForm(TIERS, "SCUBA_COURSES" as "DAYTRIP")); setEditBoatId(null); setBoatOpen(true); };
  ...
  const deleteBoat = async (id: string) => {
    if (!confirm("ลบ Scuba Course นี้?")) return;
  ...
  return (
    <div style={{ padding: "24px 20px", maxWidth: 960 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#e5e5e5" }}>🎓 Scuba Courses</h1>
      ...
      {boats.length === 0 && (
        <div style={{ textAlign: "center", color: "#333", padding: "60px 0", fontSize: 14 }}>
          ยังไม่มี Scuba Course — กด + เพิ่ม Course เพื่อเริ่ม
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex" }}>
          ...
          <div style={{ width: "min(720px,100%)", background: "#0d0d0d", borderLeft: "1px solid #1a1a1a", overflowY: "auto", display: "flex", flexDirection: "column" }}>
            <div style={{ position: "sticky", top: 0, zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "#0d0d0d", borderBottom: "1px solid #111" }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: "#ddd" }}>{editBoatId ? "แก้ไข Scuba Course" : "เพิ่ม Scuba Course"}</span>
